defmodule SmartGardenWeb.IntervalDeviceControllerTest do
  ...
  test "index intervals", %{conn: conn, device: device} do
    changeset = %Interval{name: "water", value: 100, device: device, action: "polling", index: 0}
    interval = Repo.insert!(changeset)
    conn = get(conn, device_interval_path(conn, :index, device.id))

    assert json_response(conn, 200) == %{
             "intervals" => [interval_json(interval, %{execution_schedule: nil})]
           }
  end
  ...
  test "update intervals", %{conn: conn, device: device} do
    ...
    conn =
      patch(conn, device_interval_path(conn, :update, device.id, interval.id),
        interval: %{execution_schedule: execution_schedule}
      )

    assert json_response(conn, 200) ==
             interval_json(interval, %{execution_schedule: execution_schedule})
  end

  def interval_json(interval, %{execution_schedule: execution_schedule}) do
    execution_schedule =
      if execution_schedule, do: execution_schedule, else: interval.execution_schedule
